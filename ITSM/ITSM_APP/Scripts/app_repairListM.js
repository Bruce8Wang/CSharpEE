    ...
    ShowRepairList: function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].Title.length > 100) {
                var TitleShort = data[i].Title.substr(0, 40);
            ...
            var html =
                    '<li>' +
                    '<a href="RepairBillDetailM.html?id=' + data[i].Id + '" data-ajax="false">' +
                    '<h1>提单号：' + data[i].BillNo + '</h1>' +
                    '<h2>主题：' + TitleShort + '</h2>' +
                    '<h2>报修人：' + data[i].BXEmployee + '</h2>' +
                    '<h2>处理状态：' + data[i].StatusName + '</h2>' +
                    '<h2>满意度：' + data[i].SatisfactionLevelName + '</h2>' +
                    '<h3>' + data[i].BXDate.replace('T', ' ') + '</h3>' +
                    '</a>' +
                    '</li>';
...
$(function () {
    ...
    if (localStorage.userType == 0) {
        ...
        if (listType == 1) {
            ...
            localStorage.filter = 'StatusId eq 1 ';
        }
            //我的已处理，过滤出状态为已完成的
        else if (listType == 4) {
