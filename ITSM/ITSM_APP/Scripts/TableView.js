...
var TableView = function (id) {
    ...
    this._render_framework = function () {
        ...
        //str += '<span class="filter" style="position:absolute;left:500px;top:40px;"><label>模糊查询&nbsp;</label>';
        //str += '<input type="text" style="height:100%;width:200px;border-width:0px;border-bottom-width:thin" placeholder="可对所有字段进行模糊查询" value="' + this._filter_text + '"'
    ...
    this.render = function (type) {
        ...
        for (var k in this.header) {
            ...
            buf.push('<th field="' + k + '" width="' + 20 + '%">' + self.header[k] + '</th>\n');
        }
        buf.push('<th width="10%">操作</th>');
        ...
        for (var i in rows) {
            var row = rows[i];
            var rid = row[self.dataKey];
            buf.push('<tr class="tv_row">\n<td  style="display:none" width="10"><input type="checkbox" value="');
            buf.push(rid);
            buf.push('" /></td>\n');
            ...
            buf.push('<td ><a title="编辑" href="javascript:;" onclick="storeHouse_edit(\'' + rid + '\',\'550\',\'\',\'编辑\',\'storeHouse-add.html?Id=' + rid + '\')" class="ml-5" style="text-decoration:none"><i class="icon-edit"></i>编辑</a><a title="删除" href="javascript:;" onclick="storeHouse_del(this,\'' + rid + '\')" class="ml-5" style="text-decoration:none"><i class="icon-trash"></i>删除</a></td>');
            buf.push('</tr>\n');
        }
